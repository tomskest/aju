...
- `prisma/control/schema.prisma` → the shared control-plane DB (Better-Auth,
  orgs, api keys, tenant routing). Migrations in `prisma/control/migrations/`.
- `prisma/tenant/schema.prisma` → applied to every per-org tenant DB (brains,
  vault docs, agents). Migrations in `prisma/tenant/migrations/`.
...
1. Local dev: edit `schema.prisma`, then
   `npx prisma migrate dev --create-only --schema prisma/control/schema.prisma`.
   Review the generated SQL, commit.
2. Boot (`npm start`) runs
   `prisma migrate deploy --schema prisma/control/schema.prisma` before
   starting Next.
...
3. Re-apply the raw-SQL setup files that Prisma cannot manage:
   - `prisma/tenant/vector-setup.sql` — pgvector extension + vector columns
   - `prisma/tenant/fts-setup/migration.sql` — `tsvector` + FTS triggers
   - `prisma/tenant/fts-setup/files-fts.sql` — file FTS triggers
   - `prisma/tenant/rls-policies.sql` — RLS policies + CHECK constraints
...
Those live in `prisma/tenant/*.sql` and are re-applied on every
...
npx prisma migrate diff \
  --from-schema-datasource prisma/control/schema.prisma \
  --to-schema-datamodel prisma/control/schema.prisma \
  --script > prisma/control/migrations/<timestamp>_<name>/migration.sql
```

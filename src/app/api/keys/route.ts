 ...
 */
function parseScopes(raw: unknown): { ok: true; scopes: Scope[] } | { ok: false; error: string } {
...
export async function GET(req: NextRequest) {
  ...
  const keys = rows.map((k) => ({
    ...
    organization: k.organization
      ? { id: k.organization.id, name: k.organization.name, slug: k.organization.slug }
      : null,
  }));
...
> {
  ...
  return { ok: false, status: 400, error: "no organization context to pin this key to" };
}
...
export async function POST(req: NextRequest) {
  ...
  if (body.expiresInDays != null) {
    ...
    if (days > MAX_EXPIRES_DAYS) {
      return NextResponse.json({ error: "expiresInDays too large" }, { status: 400 });
    }
    expiresAt = new Date(Date.now() + days * MS_PER_DAY);
  }

  const orgResult = await resolveKeyOrg(user.id, body.organizationId, organizationId);
  if (!orgResult.ok) {

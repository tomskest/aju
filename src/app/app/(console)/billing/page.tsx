...
import { isBillingConfigured, PLAN_LIMITS, limitsFor } from "@/lib/billing";
...
function renewalLine(sub: SubscriptionFields): string | null {
  const when = formatDate(sub.currentPeriodEnd);
  if (!when) return null;
  if (sub.cancelAtPeriodEnd) return `Access ends ${when}. No further charges.`;
  if (sub.subscriptionStatus === "canceled") return `Ended ${when}.`;
  if (sub.subscriptionStatus === "past_due" || sub.subscriptionStatus === "unpaid") {
    return `Payment failed. Stripe will retry until ${when}.`;
  }
  return `Renews ${when}.`;
}

...
- `data/control/schema.prisma`: everything listed above under "control", plus
...
- `data/tenant/schema.prisma`: the 7 tenant models (Brain, BrainAccess,
  Agent, VaultDocument, DocumentLink, VaultChangeLog, VaultFile).

Regenerate:

```bash
npx prisma generate --schema data/control/schema.prisma
npx prisma generate --schema data/tenant/schema.prisma
```

Update `package.json` scripts:

```json
"postinstall": "prisma generate --schema data/control/schema.prisma && prisma generate --schema data/tenant/schema.prisma",
...
Denormalize these columns in `data/tenant/schema.prisma`:
...
Rewrite `data/tenant/rls-policies.sql` (the port of the current file):
...
async function provisionTenant(orgId: string): Promise<Tenant> {
  ...
  //      (a) spawn `prisma migrate deploy --schema data/tenant/schema.prisma`
  ...
  //      data/tenant/vector-setup.sql        (embedding columns + HNSW)
  //      data/tenant/fts-setup/migration.sql (docs tsvector + trigram)
  //      data/tenant/fts-setup/files-fts.sql (files tsvector + trigram)
  //      data/tenant/rls-policies.sql        (brain-id RLS, Phase 1.6)
...
2. Shells out to `prisma migrate deploy --schema data/tenant/schema.prisma`
...
Runs in CI after every deploy that changes `data/tenant/`.
...
4. Drop the orphaned tables (separate migration in `data/control/migrations`):
   ```sql
...
4. Branch `neon-phase-1` in aju repo. Split `prisma/schema.prisma` into
   `data/control/schema.prisma` and `data/tenant/schema.prisma`.
...
7. Rewrite `data/tenant/rls-policies.sql` with brain-id policies
   (Phase 1.6).

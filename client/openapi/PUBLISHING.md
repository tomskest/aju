...
- npm → `@aju/sdk@0.1.0`
- PyPI → `aju==0.1.0`
- Go → `client/openapi/go/v0.1.0` (auto-pushed by the workflow)

The workflow verifies that `client/openapi/ts/package.json` and
`client/openapi/py/pyproject.toml` both declare the same version as the tag.
...
If `@aju` is taken on npm, change `name` in `client/openapi/ts/package.json`
to a scope you own (`@tomskest/aju-sdk` is the obvious fallback) and
update the docs in `client/openapi/ts/README.md`, `src/app/doc/sdks/`,
...
`client/openapi/go/v<version>`. `proxy.golang.org` picks it up the first time
anyone runs `go get github.com/tomskest/aju/client/openapi/go/ajuclient@v0.1.0`.

## Cutting a release

1. Make sure `client/openapi/openapi.yaml` and the three regenerated SDKs
   are committed and in sync. `./client/openapi/sh/generate.sh` locally if you
   need to regenerate.
2. Bump the version in **both** files to the same semver:
   - `client/openapi/ts/package.json` → `"version": "0.1.0"`
   - `client/openapi/py/pyproject.toml` → `version = "0.1.0"`

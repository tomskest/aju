---
title: The aju CLI
description: The Go CLI in apps/cli — architecture, command dispatch, device login, and how commands talk to the hosted API.
order: 40
---

# The aju CLI

The CLI is a single static Go binary. Source lives in `apps/cli/`.
...
text is hand-written at `apps/cli/cmd/help.go`. The trade is: zero external
...
`loadAuthedClient()` (`apps/cli/cmd/client.go:24-34`) returns a friendly
...
`apps/cli/internal/httpx/client.go` is a thin wrapper over `net/http` that:
...
The `x-aju-cli-version` header is what `apps/cli/cmd/precheck.go` uses to
...
`apps/cli/cmd/auth.go:60-165` implements `aju login`:
...
`apps/cli/cmd/agent_provision.go` is a near-copy of `Login` that sends
...
(`apps/cli/cmd/client.go:39-47`). Brain names are scoped to the tenant
...
`apps/cli/cmd/search.go:117-136`:
...
Errors go to stderr. `ErrSilent` (`apps/cli/cmd/client.go:15`) is used
...
Flags (`apps/cli/cmd/search.go:113-161`):
...
were linked from. From `apps/cli/cmd/search.go:163-184`:
...
`apps/cli/cmd/export.go:18-63`. Streams the response from
...
Full setup snippets: https://aju.sh/docs/mcp
```

`apps/cli/cmd/stub.go:19-29`. See [stdio-bridge.md](./stdio-bridge.md) for
...
`apps/cli/cmd/skill.go` takes a **target** (currently only `claude`) and

...
target is registered (`apps/cli/cmd/skill.go:49-57`), but the subcommand
is already structured as a target map so adding `cursor`, `cline`,
`continue`, etc. later is a matter of appending an entry.

## Install path

Hard-coded in `apps/cli/cmd/skill.go:150-156` (the `claudeSkillPath` func
...
`SkillInstall` (`apps/cli/cmd/skill.go:180-229`):
...
`apps/cli/cmd/skill.go:19-20`. Editing `apps/cli/cmd/skill_body.md` and
rebuilding the binary is the only way to change what the skill says.

## The template context

`apps/cli/cmd/skill.go:29-36`:
...
Source: `apps/cli/cmd/skill_body.md`. Key parts:
...
From `src/app/docs/claude-code/page.tsx:87-158` — the public docs spell out
...
`SkillRemove` (`apps/cli/cmd/skill.go:235-267`) deletes

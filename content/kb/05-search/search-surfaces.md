...
`apps/cli` is a Go binary that wraps the same HTTP endpoints.
Dispatch table: `apps/cli/main.go:38`.

### `aju search <query>`

`apps/cli/cmd/search.go:41`. GET `/api/vault/search`.
...
`apps/cli/cmd/search.go:77`. GET `/api/vault/semantic-search`.
...
`apps/cli/cmd/search.go:113`. GET `/api/vault/deep-search`. GraphRAG
...
`apps/cli/cmd/graph.go:14`. GET `/api/vault/backlinks`. One row per
incoming edge, printed as `path\ttitle`.

### `aju related <path>`

`apps/cli/cmd/graph.go:62`. GET `/api/vault/related`. Prints
...
`apps/cli/cmd/graph.go:109`. GET `/api/vault/graph`.

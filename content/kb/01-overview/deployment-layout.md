...
```json
// package.json
"start": "prisma db push --schema prisma/control/schema.prisma --accept-data-loss --skip-generate && tsx scripts/tenant-migrate.ts && next start"
```

The startup chain does two things before `next start`:

1. **`prisma db push --schema prisma/control/schema.prisma --accept-data-loss --skip-generate`** — syncs the control schema to `aju_control`. `--accept-data-loss` is on because the app owns the schema end-to-end; we don't preserve Prisma's migration history. The Prisma clients were generated at build time by `npm run build`, so `--skip-generate` keeps boot fast.
2. **`tsx scripts/tenant-migrate.ts`** — enumerates every `tenant.status='active'` row, acquires a per-DB advisory lock, runs `prisma db push --schema prisma/tenant/schema.prisma` against each tenant's direct DSN, re-applies `vector-setup.sql`, `fts-setup/*.sql`, and `rls-policies.sql`, then bumps the tenant's `schema_version`. Every statement is idempotent.
...
Source: `workers/install/src/index.ts`, config `workers/install/wrangler.toml`.
...
**Why a Worker and not a static page.** The installer script is rendered at request time from env vars (`GITHUB_REPO`, `BINARY_NAME`, `DEFAULT_INSTALL_DIR`) so the repo fork is swappable without rebuilds, and the version query string (`?version=cli-v0.1.0`) changes the download URL on the fly (`workers/install/src/index.ts:40-51`).

**Why a separate repo-within-the-repo.** `workers/install/package.json` has its own `wrangler` dependency; keeping it isolated means the Next.js app doesn't pull Cloudflare tooling into its node_modules.
...
|---|---|---|
| `tenant-migrate.ts` | Every boot; also manually after changes to `prisma/tenant/` | Walks every `tenant.status='active'` row, acquires an advisory lock, runs `prisma db push --schema prisma/tenant/schema.prisma` against that tenant's direct DSN, re-applies the setup SQL (vector, FTS, RLS), and bumps `schema_version`. Idempotent. |
...
```json
"db:push:control": "prisma db push --schema prisma/control/schema.prisma",
...
Source: `apps/cli/`. Not covered in depth here — see the dedicated CLI section of the KB. Relevant to deployment: GitHub Releases builds per-platform static binaries tagged `cli-vX.Y.Z`. The install worker reads the latest release via GitHub's API to populate the manifest (`workers/install/src/index.ts:113-151`).

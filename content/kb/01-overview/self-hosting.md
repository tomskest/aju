...
Source: [`github.com/tomskest/aju`](https://github.com/tomskest/aju), Apache 2.0. The in-app self-host guide lives at `/docs/self-host` (source: `src/app/docs/self-host/page.tsx`).
...
`scripts/tenant-migrate.ts` walks every active tenant on boot and brings its schema + RLS up to date. RLS policies live in `prisma/tenant/rls-policies.sql` and are re-applied alongside the schema because all the statements are idempotent (ENABLE RLS is a no-op if already set; policies are dropped + recreated).

## Optional: the install worker

If you are running a public fork with a custom install endpoint, the `workers/install/` directory is a separately deployable Cloudflare Worker. Set `GITHUB_REPO`, `BINARY_NAME`, `DEFAULT_INSTALL_DIR` in `wrangler.toml` and `wrangler deploy`.
...
- The control schema: [`prisma/control/schema.prisma`](https://github.com/tomskest/aju/blob/main/prisma/control/schema.prisma)
- The tenant schema: [`prisma/tenant/schema.prisma`](https://github.com/tomskest/aju/blob/main/prisma/tenant/schema.prisma)
- The RLS policy file: [`prisma/tenant/rls-policies.sql`](https://github.com/tomskest/aju/blob/main/prisma/tenant/rls-policies.sql)
- The CLI source: [`apps/cli/`](https://github.com/tomskest/aju/tree/main/apps/cli)
- The install worker: [`workers/install/`](https://github.com/tomskest/aju/tree/main/workers/install)

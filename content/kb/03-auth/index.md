...
- `prisma/control/schema.prisma` (control-plane `aju_control`: `User`, `Session`, `Account`, `Verification`, `ApiKey`, `DeviceCode`, `Organization`, `OrganizationMembership`, `OrganizationDomain`, `Invitation`, `AccessRequest`, `WaitlistEntry`, `Tenant`, `OAuth*`)
- `prisma/tenant/schema.prisma` (per-org `org_<cuid>`: `Brain`, `BrainAccess`, documents, etc.)

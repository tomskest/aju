...
The entire HTTP surface is Next.js App Router route handlers under `src/app/api/`. The dashboard pages live under `src/app/app/`, `src/app/doc/`, `src/app/welcome/`, etc. There is no separate Express/Fastify server.
...
Extensions loaded in every tenant DB at provision time (`data/tenant/vector-setup.sql`, `data/tenant/fts-setup/*.sql`):
...
- `data/control/schema.prisma` → `@prisma/client` (singleton in `src/lib/db.ts`, connected to `DATABASE_URL` / `CONTROL_POOLED_URL`).
- `data/tenant/schema.prisma` → `@prisma/client-tenant` (output under `node_modules/@prisma/client-tenant`). `tenantDbFor(orgId)` returns a `PrismaClientTenant` pinned to that org's encrypted DSN, cached in an LRU with a 10-minute idle reaper.
...
**Tradeoff.** Prisma does not model Postgres features we depend on (tsvector columns, vector columns, triggers, RLS policies). Those live in raw SQL files under `data/tenant/` and are applied per-tenant by `src/lib/tenant-provision.ts` and `scripts/tenant-migrate.ts`. See [deployment-layout.md](./deployment-layout.md) for the boot sequence.
...
Better-Auth owns the `user`, `session`, `account`, `verification` tables in the control DB. See `data/control/schema.prisma`. The project CLAUDE.md lists magic-link email as the primary signup path.
...
- Cloudflare Wrangler for the install worker (`worker/install/package.json:11-15`).

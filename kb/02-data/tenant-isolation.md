...
2. **Brain-id RLS inside each tenant DB.** Policies live in
   `data/tenant/rls-policies.sql` and gate on `brain_id` via the session
...
`Organization` (`data/control/schema.prisma`) lives in the control DB and
...
`data/tenant/rls-policies.sql` also installs a check constraint:

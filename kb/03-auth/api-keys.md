...
`client/cli/cmd/agent_provision.go` drives a device-code flow with
...
The override precedence, resolved in `client/cli/internal/config/config.go:190`, is: `$AJU_PROFILE` → `DefaultProfile` → `"default"`. The top-level `-p` / `--profile` flag sets `$AJU_PROFILE` before dispatch so any command picks it up uniformly.
